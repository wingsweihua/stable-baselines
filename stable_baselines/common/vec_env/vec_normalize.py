...
class GoalEnvVecNormalize(VecEnvWrapper):
    """
    A moving average, normalizing wrapper for vectorized environment.
    has support for saving/loading moving average,
    ...
    def __init__(self, venv, training=True, norm_obs=True, norm_reward=True,
                 clip_obs=10., clip_reward=10., gamma=0.99, epsilon=1e-8):
        VecEnvWrapper.__init__(self, venv)
        self.obs_rms = RunningMeanStd(shape=self.observation_space['observation'].shape)
        self.goal_rms = RunningMeanStd(shape=self.observation_space['achieved_goal'].shape)
        self.ret_rms = RunningMeanStd(shape=())
        self.clip_obs = clip_obs
        self.clip_reward = clip_reward
        # Returns: discounted rewards
    ...
    def _normalize_observation(self, obs):
        ...
        if self.norm_obs:
            if self.training:
                self.obs_rms.update(obs['observation'])
                self.goal_rms.update(obs['achieved_goal'])
            for key, rms in zip(['observation', 'achieved_goal', 'desired_goal'],
                                [self.obs_rms, self.goal_rms, self.goal_rms]):
                obs[key] = np.clip((obs[key] - rms.mean) / np.sqrt(rms.var + self.epsilon), -self.clip_obs,
                              self.clip_obs)
            return obs
        else:
            return obs
    ...
    def save_running_average(self, path):
        ...
        :param path: (str) path to log dir
        """
        for rms, name in zip([self.obs_rms, self.goal_rms, self.ret_rms], ['obs_rms', 'goal_rms', 'ret_rms']):
            with open("{}/{}.pkl".format(path, name), 'wb') as file_handler:
                pickle.dump(rms, file_handler)
    ...
    def load_running_average(self, path):
        ...
        :param path: (str) path to log dir
        """
        for name in ['obs_rms', 'goal_rms', 'ret_rms']:
            with open("{}/{}.pkl".format(path, name), 'rb') as file_handler:
                setattr(self, name, pickle.load(file_handler))
